'''
projeto de calculadora de financiamentos
'''
def lambda_handler():
    ...
    var_carteira_bruta_atual = var_carteira_bruta0
    var_renda0 = var_carteira_bruta0 * (var_tx_juros / 100)
    var_renda_atual = var_renda0
    list = []
    carteira_bruta_renda_json = {
        "carteira_bruta": var_carteira_bruta0,
        "renda": var_renda0
    }
    list.append(carteira_bruta_renda_json)
    print(list)
    var_qtd_parcelas -=1
    for row in range(var_qtd_parcelas):
        var_carteira_final = var_carteira_bruta_atual + var_renda_atual
        var_renda_final = (var_carteira_final - v_valor_parcela) * (var_tx_juros / 100)
        carteira_bruta_renda_json = {
            "carteira_bruta": var_carteira_final,
            "renda": var_renda_final
        }
        var_carteira_bruta_atual = var_carteira_final
        list.append(carteira_bruta_renda_json)
        print(row)
        print(list)
    return None
